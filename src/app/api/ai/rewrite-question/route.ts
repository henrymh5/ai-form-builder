...
import { rewriteQuestion } from "@/lib/ai/functions/rewrite-question";
import { rewriteQuestionInputSchema } from "@/lib/ai/schemas";
import { checkRateLimit, AI_RATE_LIMITS } from "@/lib/db/repositories/rate-limit";
import { AppError, isAppError } from "@/lib/errors";

const requestSchema = rewriteQuestionInputSchema.extend({
  workspaceId: z.string().uuid(),
  formId: z.string().uuid().optional(),
});

/** POST /api/ai/rewrite-question (plan §11 "einzelne Frage bearbeiten"). */
export async function POST(request: Request) {
  try {
    ...
    const result = await rewriteQuestion(
      { label: parsed.data.label, description: parsed.data.description, style: parsed.data.style },
